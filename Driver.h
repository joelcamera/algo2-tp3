...
class Driver
{
  ...
  private:
     ...
     ************************************************************************/
     ...
     BaseDeDatos base;
     Driver::Dato toDriverDato(const dato& d) const;
     dato fromDriverDato(const Driver::Dato& d) const;
     aed2::Dicc<NombreCampo, Driver::Dato> toDriverReg(const registro& reg) const;
     registro fromDriverReg(const aed2::Dicc<NombreCampo, Driver::Dato>& reg) const;
     bool tieneIndiceDeTipo(const NombreTabla& tabla, bool nat, NombreCampo& nombre) const;
     Dicc<NombreTabla, NombreCampo> indicesNat;
     Dicc<NombreTabla, NombreCampo> indicesStr;
